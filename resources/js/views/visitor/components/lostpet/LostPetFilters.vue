<template>
  ...
  >
    ...
    <h2
      ...
      style="
        background-image: url('images/arxiki-eikona-anazitisi-idioktiton-apo-katikidia.jpg');
      "
    >
    ...
    >
      <div class="m-mb--sm">
        <a href="/home" class="color--dark">Αρχική σελίδα</a>&nbsp;>&nbsp;<span
          class="color--brand"
          >Αναζήτηση Χαμένου Κατοικιδίου</span
        >
      </div>
    ...
    <div class="section--boxed col--100 cols--um">
      ...
      <div
        v-for="(lostPet, index) in lostPets.data"
        :key="index"
        class="col--70 adoption-posts-list m-text--center before--load"
        :class="customEffect"
      >
        <div
          class="col--30 m-col--100 adoption-posts-post p--sm mr--sm mb--sm"
          :class="{ el__hidden: showfilters }"
        >
          <a href="/post">
            <img :src="lostPet.primaryImageUrl" width="100%" />
          </a>
          <div class="font--20 mt--sm weight--700">
            <a href="/post">{{ lostPet.name }}</a>
          </div>
          <div class="font--12 pb--sm">
            <a href="/blog">{{ lostPet.region }}</a>
          </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      getLostPets: "lostPets/getLostPetsFront",
